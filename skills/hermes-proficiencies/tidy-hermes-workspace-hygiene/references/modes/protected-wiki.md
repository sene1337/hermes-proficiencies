...
Use when the task touches the owner's private knowledge-base wiki, private knowledge-base wiki, personal knowledge base, or any protected knowledge base path.

### 5. Protected private knowledge-base Wiki Rule

Treat private knowledge-base wiki directories as protected knowledge bases, not ordinary workspaces.

Default stance:
- **Read-only by default.** Searching, reading, quoting, and summarizing are allowed when directly relevant.
- **No edits, deletes, moves, renames, rewrites, bulk formatting, cleanup, or file creation inside private knowledge-base wiki paths** unless the user gives explicit, current-task permission for that exact scope.
- **No inferred permission.** A broad request like "work on my private knowledge-base OS" or "clean up docs" does not authorize touching wiki files.
